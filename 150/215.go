...
// PrioriQueue 大根堆
type PrioriQueue struct {
	q   []int
	pos int //指示最后一个元素的下标
}

func (pq *PrioriQueue) Len() int {
	return len(pq.q)
}
func (pq *PrioriQueue) Insert(val int) {
	//if pq.Len() == 2 {
	//	pq.q[1] = val
	//	return
	//}
...
func (pq *PrioriQueue) Top() int {
	return pq.q[1]
}
func (pq *PrioriQueue) Pop() []int {
...
func NewPrioriQueue() *PrioriQueue {
	return &PrioriQueue{
		q:   make([]int, 1),
		pos: 0,
	}
